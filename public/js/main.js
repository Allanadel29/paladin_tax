...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function checkEmailValidity(email) {
      ...
      // Try primary validation with timeout
      try {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 3000); // 3 second timeout
        
        const response = await fetch(`https://emailvalidation.abstractapi.com/v1/?email=${encodeURIComponent(email)}`, {
        ...
        if (response.ok) {
          ...
          if (data.deliverability === 'UNDELIVERABLE') {
            return { valid: false, message: 'This email address appears to be invalid or does not exist.' };
          }
          
          if (data.is_smtp_valid?.value === false) {
            return { valid: false, message: 'This email address could not be verified. Please try another.' };
          }
        ...
        console.warn('Primary validation service failed, trying fallback');
      }

      // Try fallback: Check MX records
      try {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 3000);
        
        const mxResponse = await fetch(`https://api.mxer.io/v1/mxlookup?domain=${encodeURIComponent(domain)}`, {
          signal: controller.signal
        });
        ...
        if (mxResponse.ok) {
          ...
          if (mxData.mxRecords && mxData.mxRecords.length > 0) {
            return { valid: true, message: 'Email domain is valid' };
          } else {
            return { valid: false, message: 'Domain does not have valid mail servers. Email likely invalid.' };
          }
        }
      } catch (error) {
        console.warn('MX lookup failed, using format validation only');
      }

      // Final fallback: If all APIs fail, accept email if format is valid and not disposable
      // This ensures legitimate users aren't blocked by API failures
      return { valid: true, message: 'Email format is valid (verification service unavailable)' };
    }
    ...
    if (contactFormElement) {
      contactFormElement.addEventListener('submit', async (e) => {
        ...
        // Get form data
        const formData = {
          name: contactFormElement.querySelector('input[name="name"]').value.trim(),
          email: contactFormElement.querySelector('input[name="email"]').value.trim(),
          subject: contactFormElement.querySelector('input[name="subject"]').value.trim(),
          message: contactFormElement.querySelector('textarea[name="message"]').value.trim()
        };
        
        // Validate email
        const emailCheck = await checkEmailValidity(formData.email);
        if (!emailCheck.valid) {
          showFormMessage(`Email Validation Error: ${emailCheck.message}`, 'error');
          return;
        }
        ...
        try {
          ...
          showFormMessage('Error sending message. Please try again or contact us directly at paladin.taxandtransferpricing@gmail.com', 'error');
        } finally {
          // Reset button state
    ...
    function showFormMessage(message, type) {
      ...
      formMessage.style.color = type === 'success' ? 'rgb(16, 182, 233)' : '#dc3545';
      formMessage.style.padding = '10px';
      formMessage.style.borderRadius = '4px';
      formMessage.style.backgroundColor = type === 'success' ? '#cff0ff' : '#f8d7da';
      formMessage.style.border = `1px solid ${type === 'success' ? 'rgb(16, 182, 233)' : '#f5c6cb'}`;
